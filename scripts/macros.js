function maketapirwikiparseandreplace() {
        ...
        function parseandreplace(tnode) {
                if (tnode.type == "#wikilink") {
                    ...
                    var topicMatches = tnode.value.match(tokens.topicMacro.re);
                    ...
                    for (match in topicMatches) {
                        ...
                        tnode.value = tnode.value.replace(tokens.topicMacro.re, topicResult);
                    }
                    tnode.value = tnode.value.replace(tokens.recentChangesMacro.re, tokens.recentChangesMacro.sub());
                    var includeMatches = tnode.value.match(tokens.includeMacro.re);
                    ...
                    for (match in includeMatches) {
                        ...
                        tnode.value = tnode.value.replace(tokens.includeMacro.re, includeResult);
                    }
                    ...
                    for (var i = 0; i < tnode.siblings.length; i++) {
                        parseandreplace(tnode.siblings[i]);
                    } // contents
                }
            } // parseandreplace()
