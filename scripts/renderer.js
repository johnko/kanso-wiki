...
function makehtmlrenderer() {
        ...
        function makeattributestring(s, tag) {
            ...
            if (tag === "img") {
                ...
                var ta2 = /(<>|=|<|>)/.exec(s);
                if (ta2) {
                    st += "text-align:" + align[ta2[1]] + ";";
                }
                var va = /(~|\^)/.exec(s);
                if (va) {
                    st += "vertical-align:" + align[va[1]] + ";";
                }
            }
            // explicit styles
            var ss = /\{(.+)\}/.exec(s);
            ...
            var colspan = /\\(\d+)/.exec(s);
            if (colspan) {
                at += qat('colspan', colspan[1]);
            }
            var rowspan = /\/(\d+)/.exec(s);
            if (rowspan) {
                at += qat('rowspan', rowspan[1]);
            }
            return at;
        }
        ...
        function html2entities(sometext) {
            var re = /[<>"'&]/g;
            return sometext.replace(re, function(m) {
